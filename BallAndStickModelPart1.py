...
class BallAndStick:
    ...
    def _setup_morphology(self):
        self.soma = h.Section(name='soma', cell=self) # cell body
        self.dend = h.Section(name='dend', cell=self) # dendrite
        self.all = [self.soma, self.dend]
        self.dend.connect(self.soma)
        self.soma.L = self.soma.diam = 12.6157 # soma length = diameter = 12.6157
        self.dend.L = 200
        self.dend.diam = 1
    def _setup_biophysics(self):
        ...
        for seg in self.soma:
            seg.hh.gnabar = 0.12  # Sodium conductance in S/cm2, S is in Siemens
...
h.finitialize(-65 * mV)
h.continuerun(25 * ms) # starts at 0 and simulates to 25

#from bokeh.io import output_notebook # not working??
